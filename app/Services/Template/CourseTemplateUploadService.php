...
class CourseTemplateUploadService
{
    public function handle(UploadedFile $uploadedFile, ?int $courseId = null): array
    {
        ...
        try {
            ...
            // Prefer the editable HTML generated by pdftohtml. The previous flow discarded
            // this HTML and used a full-page rasterization as the frame, making every text
            // element non-editable in the imported template.
            $frontFrameSrc = null;
            $backFrameSrc = null;
            ...
            // Keep the generated page and its editable text blocks. Only use the empty
            // container fallback when the converter did not produce usable HTML.
            $frontTemplate = $templateHtml !== ''
                ? $templateHtml
                : '<div class="cert-container" style="position: relative; width: 100%; height: 100%;"></div>';
            $backTemplate = null;
            ...
            $frontTemplate = $this->applyEditorScaleToFrontTemplate(
                $this->injectBaseStyles($frontTemplate, $orientation),
                $orientation
            );
            if ($backTemplate !== null) {
                $backTemplate = $this->applyEditorScaleToFrontTemplate(
                    $this->injectBaseStyles($backTemplate, $orientation),
                    $orientation
                );
            }
...
";
    ...
    /**
     * Extract only the page-sized background images from pdftohtml output.
     * Text paragraphs and smaller assets such as QR codes remain in the HTML.
     *
     * @return array{0: string, 1: array<int, string>}
     */
    private function extractPdfFramesAndRemoveFromTemplate(string $html): array
    {
        ...
        foreach ($pages as $page) {
            ...
            $candidate = null;
            $candidateArea = 0.0;
            foreach ($xpath->query('.//img', $page) as $imgNode) {
                ...
                $width = $width > 0 ? $width : $this->extractPixelsFromStyle($style, 'width');
                $height = $height > 0 ? $height : $this->extractPixelsFromStyle($style, 'height');

                $isPageSized = $width >= ($pageWidth * 0.85)
                    && $height >= ($pageHeight * 0.85);
                if (! $isPageSized || ($width * $height) <= $candidateArea) {
                    continue;
                }
            ...
            $frames[] = trim((string) $candidate->getAttribute('src'));
            $candidate->parentNode?->removeChild($candidate);
        }

        $updatedHtml = $this->saveHtmlUtf8($dom);
        return [trim($updatedHtml) !== '' ? $updatedHtml : $html, $frames];
    }
