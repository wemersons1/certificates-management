...
interface ConfirmDataScreenProps {};

const ConfirmDataScreen: FC<ConfirmDataScreenProps> = () => {
    const [code, setCode] = useState('');
    const { setUserLogged, setUserToken, signOut, user } = useContext(AppContext);
    const [errorConfirmationCode, setErrorConfirmationCode] = useState(false);
    const [timeToResendCode, setTimeToResendCode] = useState(60);
    const navigate = useNavigate();
    
    useEffect(() => {
        setInterval(() => {
            setTimeToResendCode(prev => {
                if (prev === 0) return 0;
                return prev - 1;
            })
        }, 1000);
    ...
    const handleResendCode = () => {
        setCode('');
        setErrorConfirmationCode(false);
        setTimeToResendCode(60);
        api.post('/resend-confirmation-code').then(response => {
            toast.success(`Código de confirmação reenviado! Verifique o e-mail ${user.email}.`);
        });
    };
    ...
    const handleSubmit = (e: { preventDefault: () => void;}) => {
        ...
        api.post('/validate-code', data).then(response => {
        ...
        <Fragment>
            ...
            <section style={{
                ...
                <div style={{
                    ...
                    >
                    Enviamos um código de 4 dígitos para o e-mail <strong>{user.email}</strong>. Por favor, insira o código para confirmar sua conta.
                    <br />
                    <span>
                    ...
                    <form style={{ marginBottom: '1.5rem' }} onSubmit={handleSubmit}>
                        <div style={{ marginBottom: '1.5rem' }}>
                            <label htmlFor="confirmationCode" style={{
                                display: 'block',
                                textAlign: 'left',
                                marginBottom: '0.5rem',
                                color: '#18306b',
                            ...
                            <input
                                ...
                                required
                                className={errorConfirmationCode ? 'border-danger' : ''}
                                style={{
                                    width: '100%',
                                    padding: '0.75rem',
                                    fontSize: '1.5rem', // Larger for the code
                                    letterSpacing: '0.5em', // Space out digits
                                    textAlign: 'center',
                                    borderRadius: '8px',
                    ...
                    <button
                        disabled={timeToResendCode > 0}
                        onClick={handleResendCode}
                        className={timeToResendCode > 0 ? '' : 'text-primary'}
                        style={{
                            background: 'none',
                            border: 'none',
                            fontSize: '1rem',
                    ...
                    >
                         Reenviar Código {timeToResendCode ? `(${timeToResendCode})` : ''}
                    </button>
                </div>
            </section>
